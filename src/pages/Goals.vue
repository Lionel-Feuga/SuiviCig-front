...
const handleDateSelection = (newDates) => {
  if (newDates.length <= 2) {
    ...
    const newDate = newDates.find(
      (date) => !selectedDates.value.includes(date)
    );
    if (newDate) {
      const newTimestamp = new Date(newDate).getTime();
      const existingTimestamps = selectedDates.value.map((d) =>
        new Date(d).getTime()
      );
      const differences = existingTimestamps.map((timestamp) =>
...
const saveRange = async () => {
  if (
    !range.value.start ||
    !range.value.end ||
    maxCigarettes.value === null ||
    maxCigarettes.value < 0
  ) {
    ...
    <v-card width="600" outlined>
      <v-card-text>
        ...
        <v-text-field
          ...
          :style="{ width: '100%' }"
        />
        <v-btn block color="primary" class="mt-4" @click="saveRange">
